...
export class CotationRowComponent implements OnDestroy, OnChanges {
  ...
  setInitialCotation(): void {
    if (!this.router.url.includes('panier/commander/valider')) {
      this.cartService.changeCotation(this.cotations[0], this.produit.reference);
      this.activeCotation.next(this.cotations[0]);
      if (this.cartService.cart.items[this.produit.reference]?.cotation != null) {
        let lowestPriceCotation = this.cotations[0];
        let lowestPriceIndex = 0;

        for (let i = 0; i < this.cotations.length; i++) {
          if (this.cotations[i].prixvente < lowestPriceCotation.prixvente &&
            this.cotations[i].produit == this.cartService.cart.items[this.produit.reference].cotation.produit) {
            this.cartService.changeCotation(this.cotations[i], this.produit.reference);
            this.activeCotation.next(this.cotations[i]);
            lowestPriceCotation = this.cotations[i];
            lowestPriceIndex = i;
          }
        }

        this.indexOfCot = lowestPriceIndex;
        this.activeCotation.next(lowestPriceCotation);
      }
    }
  }
