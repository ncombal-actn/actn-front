...
export class RollingHeaderComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    if (isPlatformBrowser(this.platformId)) {
      this.showHeader = this.windowService.pageYOffset > 120;
      ...
      this.ngZone.runOutsideAngular(() => {
        fromEvent(this.windowService.nativeWindow.document, 'scroll')
          ...
          .subscribe(e => {
            if (!this.showHeader && this.windowService.pageYOffset > 120) {
              this.ngZone.run(() => this.showHeader = true);
            } else if (this.showHeader && this.windowService.pageYOffset <= 120) {
