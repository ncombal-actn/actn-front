...
export class SystemeAideChoixComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.userService.getCategories().subscribe(ret => {
            ...
            const setNiveau = (arrayDeMarque: Array<{ libelle: string, code: string, photo: string; }>, map: Map<string, any>) => {
        ...
        this.userService.getFiltres(this.marqueActive.code).subscribe(ret => {
            ...
            this.source2.forEach(element => {
                if (this.mapFiltreCom.has(element.val01)) {
                    if (element.val02) {
                        this.mapFiltreCom.get(element.val01).subcat.set(element.val02, { photo: element.val02photo, sequence: +element.sequence, color: element.couleur, lib: element.couleurlib });
                    }
                } else {
                    this.mapFiltreCom.set(element.val01, { photo: element.val01photo, sequence: +element.sequence, color: element.couleur, lib: element.couleurlib, subcat: new Map() });
                    if (element.val02) {
                        this.mapFiltreCom.get(element.val01).subcat.set(element.val02, { photo: element.val02photo, sequence: +element.sequence, color: element.couleur, lib: element.couleurlib });
                    }
                }
    ...
    getFiltres() {
        ...
        this.filtres.forEach(filtre => {
            ...
            filtre.options = Array.from(set).map((el: string) => { return { value: el, count: this.countFiltreValue(this.filterProduit(), filtre.num, el) } }).sort((e1, e2) => e1.value.localeCompare(e2.value));
        });
