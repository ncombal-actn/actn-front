...
export const ONLYWAN_ROUTES: Routes = [
  {
    ...
    children: [
      ...
      },
      {
        canActivate: [AuthGuard],
        path: 'mesClients',
        loadChildren: () => import('./mes-clients/mes-clients.routes').then(m => m.MES_CLIENTS_ROUTES)
      },
      {
        canActivate: [AuthGuard],
        path: 'maConsommation',
        loadChildren: () => import('./ma-consomation/ma-consomation.routes').then(m => m.MA_CONSOMMATION_ROUTES)
      },
      {
        path: 'contacts',
